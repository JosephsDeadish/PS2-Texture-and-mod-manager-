...
def load_catalogue(
    ...
    *,
    ...
    list[dict]
        ...
        :data:`_TYPE_FILES`.  Each entry dict always contains every field
        listed in :data:`_REQUIRED_FIELDS` and :data:`_OPTIONAL_DEFAULTS`,
    ...
    for type_str, filename in _TYPE_FILES.items():
        path = base / filename
        if not path.exists():
